...
def send_message():
    ...
    if 'hello' in u:
        bot = Label(chat_bg, height=1, width=64, bg='white', fg='black',text=' Bot> Hello how can I help you.', font=12, anchor='w')
        bot.place(x=xi, y=yi+25)
    elif 'how are you?' in u:
        bot = Label(chat_bg, height=1, width=64, bg='white', fg='black', text=' Bot> Im fine what about you', font=12, anchor='w')
        bot.place(x=xi, y=yi + 25)
    elif 'What is Python?' in u:
        bot = Label(chat_bg, height=1, width=64, bg='white', fg='black', text=' Bot> Python is a programming language.', font=12, anchor='w')
        bot.place(x=xi, y=yi + 25)
    elif 'good morning' in u:
        bot = Label(chat_bg, height=1, width=64, bg='white', fg='black', text=' Bot> A very warm good morning', font=12, anchor='w')
        bot.place(x=xi, y=yi + 25)
    elif 'What is the Weather in Vadodara today?' in u:
        bot = Label(chat_bg, height=1, width=64, bg='white', fg='black', text=' Bot> Partly cloudy. Hazy. High 34°C.', font=12, anchor='w')
        bot.place(x=xi, y=yi + 25)
    elif 'When was Google developed ?' in u:
        bot = Label(chat_bg, height=1, width=64, bg='white', fg='black', text=' Bot> On September 4, 1998', font=12, anchor='w')
        bot.place(x=xi, y=yi + 25)
    elif u == 'clear':
        clear_message()
    else:
